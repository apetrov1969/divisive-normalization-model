function [retval_mainResults,...
          retval_allResults] = Explore_Size_by_Orient(M, specs_simulation)
      
%Explore_Size_by_Orient - Size tuning (orientation)
%
% Visual stimuli:
%  Luminance gratings with various sizes and orientations
%
% Refrerences
...
specs_images.orientLw_deg = specs_images.orient_deg - specs_simulation.fwhh_orient_deg/2; % lower  ori at half-height
specs_images.orientHg_deg = specs_images.orient_deg + specs_simulation.fwhh_orient_deg/2; % higher ori at half-height
...
numConditions = [size(specs_images.condition_radius_deg,2),3]; % [imgSize,imSpf(opt,ori_low,ori_high)]
...
for c1=1:numConditions(1) % size
    ...
    c2=2; % Ori_low
    specs_grating.orientation_deg = specs_images.orientLw_deg; % lower ori at half-height
    specs_grating.frequency       = specs_images.spFreq_cpd;
    specs_grating.amplitude = stS.rangeLum/2;
    baseGrating = Grating2D(specs_grating,gridX,gridY)+stS.bgLum;
    tempImg=baseGrating;
    ...
    c2=3; % Ori_high
    specs_grating.orientation_deg = specs_images.orientHg_deg; % higher ori at half-height
    specs_grating.frequency       = specs_images.spFreq_cpd;
    specs_grating.amplitude = stS.rangeLum/2;
    baseGrating = Grating2D(specs_grating,gridX,gridY)+stS.bgLum;
    tempImg=baseGrating;
...
results = reshape(results,[num_ev_type,numConditions(1),numConditions(2)]); % [neTyp,imSize,imOri(opt,low,high)]

%- Find peaks
peak_x=NaN(num_ev_type,numConditions(2)); % [neTyp,imOri]
peak_y=NaN(num_ev_type,numConditions(2)); % [neTyp,imOri]
for c2=1:numConditions(2) % imOri(opt,low,high)
...
retval_mainResults.name = 'Disk size x Ori';
...
retval_mainResults.legend_items = {'Opt';'Ori_Low';'Ori_High'};
...
%- Set 'comment'
retval_mainResults.comment = sprintf( ['Plot the results of ''Explore_Size_by_Orient'' by calling ''retval_mainResults.plot(tp)''.\n'...
