function [retval_mainResults,...
          retval_allResults] = Explore_SpFreq_by_Size(M, specs_simulation)

%Explore_SpFreq_by_Size - Spatial-frequency tuning (size)
%
% Visual stimuli:
%  Luminance gratings with various frequencies and sizes
%
% References: I couldn't find any study.
%  Osaki et al. (2011)
...
if(specs_images.base_radius_deg*4>maxGridR_deg)
    warning('M.stim_spec.imageSize_pix is too small for ''Explore_SpFreq_by_Size''');
end
...
specs_images.condition_spFreq_oct = -1:0.05:2.5;
...
numConditions = [size(specs_images.condition_spFreq_cpd,2),...
                 size(specs_images.condition_radius_deg,2)]; % [imSpF,imRad];

% Generate stimuli
setStimuli = NaN([stS.imageSize_pix, numConditions(1), numConditions(2)]); % [x,y,imSpF,imRad]
...
for c1=1:numConditions(1) % spatial frequency
    for c2=1:numConditions(2) % size
...
results = reshape(results,[num_ev_type,numConditions(1),numConditions(2)]); % [neTyp,imSpf,imSize]
...
retval_mainResults.name = 'Spatial frequency x Size';
...
retval_mainResults.command = [  'figure(''Name'',retval_mainResults.name);'...
                                ...
                                'hold off;'...
                                'retval_mainResults.bandwidth_oct(temp_type,:)'...
                            ];
retval_mainResults.plot = @(tp) eval( strrep( retval_mainResults.command, 'temp_type', num2str(eval(strcat(tp))) ) );

%- Set 'comment'
retval_mainResults.comment = sprintf( ['Plot the results of ''Explore_SpFreq_by_Size'' by calling ''retval_mainResults.plot(tp)''.\n'...
