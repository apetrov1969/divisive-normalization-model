function [retval_mainResults,...
          retval_allResults] = Explore_Contrast_by_SpFreq(M, specs_simulation)

%Explore_Contrast_by_SpFreq - Contrast sensitivity (spatial-frequency)
%
% Visual stimuli:
%  Luminance gratings with various contrasts and spatial-frequencies
%
% References:
...
specs_images.condition_spFreq_oct = 0:0.5:2;
...
numConditions = [size(specs_images.condition_contrast,2),...
                 size(specs_images.condition_spFreq_oct,2)]; % [imCon,imSpf]

%- Generate stimuli
setStimuli = NaN([stS.imageSize_pix, numConditions]); % [x,y,imOri,imSpf]
...
for c1=1:numConditions(1) % contrast
    ...
    for c2=1:numConditions(2) % spatial frequency
        specs_grating.frequency = specs_images.condition_spFreq_cpd(c2);
...
results = reshape(results,[num_ev_type,numConditions(1),numConditions(2)]); % [neTyp,imCon,imSpf]

%- Set 'retval_mainResults'
retval_mainResults.name = 'Contrast by SpFreq';
...
retval_mainResults.legend_items = cellstr(num2str(specs_images.condition_spFreq_cpd','%+.2f c/d'));
...
%- Set 'comment'
retval_mainResults.comment = sprintf( ['Plot the results of ''Explore_Contrast_by_SpFreq'' by calling ''retval_mainResults.plot(tp)''.\n'...
