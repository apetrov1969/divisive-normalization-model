function [retval_mainResults,...
          retval_allResults] = Explore_SurrXSpFreq_Supp(M, specs_simulation)

%Explore_SurrXSpFreq_Supp - Comparing spatial-frequency tuning of Cross/Surr suppression
%
% Visual stimuli:
...
for c=1:numConditions(1) % spatial frequency
    ...
    %Revision: tempImg(gridR_deg<specs_images.radius_Cent_deg)=baseGrating(gridR_deg<(specs_images.radius_Cent_deg));
...
results = squeeze(resp_SuppDrv(idx_neuron_orient,idx_neuron_spFreq,idx_neuron_rfLoc,:));
results = reshape(results,[1, numConditions(1), numConditions(2)]);
