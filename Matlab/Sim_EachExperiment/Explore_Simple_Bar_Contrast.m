function [retval_mainResults,...
          retval_allResults] = Explore_Simple_Bar_Contrast(M, specs_simulation)

%Explore_Simple_Bar_Contrast - Measuring simple-cell receptive field with a bar (position x contrast)
%
% Visual stimuli:
...
specs_images.barWidth = x_step;
%specs_images.condition_contrast = [0.12, 0.25, 0.5, 1.0];
specs_images.condition_contrast = [0.03, 0.06, 0.12, 0.25, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0];

specs_images.condition_positionX_deg = -maxPosX:x_step:+maxPosX;

numConditions = [size(specs_images.condition_positionX_deg,2),...
                 size(specs_images.condition_contrast,2), 2]; % [imPosX,imBarW,Light/Dark]

%- Generate stimuli
setStimuli = NaN([stS.imageSize_pix, numConditions]);
for px=1:numConditions(1) % position
    for cn=1:numConditions(2) % contrast
        half_width=specs_images.barWidth/2;

        tempImg = NaN([stS.imageSize_pix]);
        tempImg(:)=specs_images.back;
        tempColor = (specs_images.light-specs_images.back) * specs_images.condition_contrast(cn) + specs_images.back;
        tempImg(abs(gridX-specs_images.condition_positionX_deg(px)) < half_width) = tempColor;
        setStimuli(:,:,px,cn,1) = tempImg;

        tempImg = NaN([stS.imageSize_pix]);
        tempImg(:)=specs_images.back;
        tempColor = (specs_images.dark-specs_images.back) * specs_images.condition_contrast(cn) + specs_images.back;
        tempImg(abs(gridX-specs_images.condition_positionX_deg(px)) < half_width) = tempColor;
        setStimuli(:,:,px,cn,2) = tempImg;
    end
end
...
sum_sqdiff = cell(1,5);
for cn=1:numConditions(2) % width
    temp_results = squeeze(results(:,:,cn,3)); % [neTyp,imPosX,imBarW,Light/Dark/L-D] -> [neTyp,imPosX]
    ...
    derived_fwhh_oct(:,cn) = hh_right-hh_left;
    derived_cent_cpd(:,cn) = cent_cpd;
end


%% Set 'retval_mainResults'
retval_mainResults.name1 = 'Bar position x contrast';
...
retval_mainResults.name2 = 'Bandwidth (FWHH) x Image probe (Contrast)';
retval_mainResults.xval2 = specs_images.condition_contrast;
...
retval_mainResults.xval3 = specs_images.condition_contrast;
retval_mainResults.xlabel3 = 'Contrast';
retval_mainResults.ylabel3 = 'Derived frequency (oct)';
...
retval_mainResults.command = [
                                ...
                                'semilogx(retval_mainResults.xval2,squeeze(retval_mainResults.fwhh_oct(temp_type,:)),''-'');'...
                                'axis([0.01 1 0 3]);'...
                                ...
                                'semilogx(retval_mainResults.xval3,log2(squeeze(retval_mainResults.cent_cpd(temp_type,:))),''-'');'...
                                'axis([0.01 1 0 2.5]);'...
